...
# Printer #
###########
def print_solution(data, routing, assignment, coordinatesMap,addresses):
...
def main_function(num_vehicles, names, addresses,coordinatesMap):
  ...
  if assignment:
    return print_solution(data, routing, assignment, coordinatesMap, addresses)
...
def maps():
  ...
  if(result["typeOfRouting"] == "distance"):
    ...
    for x in range(len(optimal_routes_list)-1):
      ...
      ref.put('/', 'routes/26Feb2019/' + str(x), obj)
    '''
    ...
    return render_template("result.html", optimal_routes=optimal_routes, addresses= addresses,manager_map= manager_map)
  else:
    return "yet to do "
...
def rendermap():
  ...
  # print(result[0][0])
  return render_template("maps.html",result = result)
